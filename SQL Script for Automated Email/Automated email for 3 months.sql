-- This is the SQL Script to send automated email to users with expiring programme in 3 months

DECLARE @START_DT DATE = (SELECT DATEADD(MONTH, DATEDIFF(MONTH,0,GETDATE())+3,0))
DECLARE @END_DT DATE = (SELECT DATEADD(MONTH,DATEDIFF(MONTH,0,GETDATE())+4,-1))
...
SET @body = '<html><body><p>Greetings,<br> <br>You are getting this reminder email as one of the programme under your faculty is going to expire soon. Please refer to the table in the email for more details. <br><br>Please be reminded that the following programmes are approaching the deadline for the accreditation process in 3 months time. As such, please ensure that your report for the programme under your faculty is done and compiled in one pdf document. Please submit the documents through the Program Academic Accreditation System as soon as possible. Your cooperation is highly appreciated. <br><br> Best regards,<br>Quality Management and Enhancement Centre (QMEC) Team</p>
<table border = 1>
<tr>
