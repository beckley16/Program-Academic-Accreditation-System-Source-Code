-- This is the SQL Script to send automated email to users with expiring programme in 1 month

DECLARE @START_DT DATE = (SELECT DATEADD(MONTH, DATEDIFF(MONTH,0,GETDATE())+1,0))
DECLARE @END_DT DATE = (SELECT DATEADD(MONTH,DATEDIFF(MONTH,0,GETDATE())+2,-1))
