...
import moment from "moment"; //to work with date
...
const MyLogPage = () => {
  ...
  const getLogs = async () => {
    try {
      setLoading(true);
      const response = await axiosInstance.get<ILogDto[]>(MY_LOGS_URL); //get is the method for return in typescript get the logs dto from the my logs url using the axios instance with url of the api
      const { data } = response;
      setMyLogs(data);
      setLoading(false);
    } catch (error) {
  ...
  return (
    <div className="pageTemplate2">
      <h1 className="text-2x1 font-bold">My Logs</h1>
